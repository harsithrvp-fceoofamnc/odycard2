...
### 13 Jun 2026
- **Swapped to cleaner doodle (`doodle2.png`)**: replaced `public/ody/chat-bg.jpg` with a recoloured, softened version of the new sparser doodle (`public/doodle2.png`) — the first one was too busy. Same recolour recipe (now ~180 KB, slightly more visible). Old `public/annapoorna_doodle_art.png` is now unused and can be deleted.
- **Custom Annapoorna doodle chat wallpaper** (`public/ody/chat-bg.jpg` + template `#chat` CSS). User supplied a B&W Coimbatore/Annapoorna doodle illustration (`public/annapoorna_doodle_art.png`). Recoloured it to the theme (walnut-brown lines on warm beige) and made a softened web-optimised version (`public/ody/chat-bg.jpg`, ~320 KB, 1100px) used as the chat background (cover, faded ~⅓ with a beige overlay so messages stay readable). Replaced the earlier hand-drawn SVG doodle pattern. Recolour done with ImageMagick `-level-colors '#5e3a1c','#f6eedd'` + colorize. *Why: user wanted their own doodle art as the chat background, matching the beige/brown/gold theme.*
