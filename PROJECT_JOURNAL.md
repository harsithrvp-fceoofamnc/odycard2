...
### 13 Jun 2026
- **Tap-to-zoom dish photo (lightbox)** (template). Tapping a dish card's photo opens a centered **lightbox** over a dark dimmed (blurred) backdrop showing the enlarged image + bestseller tag + name + price + an "+ Add to order" button (adds with the vibration buzz and closes). Close via the ✕ or tapping the backdrop. New `#lb` overlay (z-index 12), `zoomDish(id)`/`closeLb()`. Image is the Annapoorna logo placeholder until real dish photos are added. *Why: user wanted tapping a dish photo to enlarge it with a darkened background.*
