...
// odysra.com/annapoorna/supervisor — sample SUPERVISOR view (one branch).
...
const BRANCHES: Branch[] = [
  { name: "RS Puram", slug: "rs-puram", orders: 4120, aov: 124, rating: 4.6, topDish: "Ghee Roast", lang: "Tamil 62%", busy: "8–9 AM" },
  { name: "Gandhipuram", slug: "gandhipuram", orders: 4480, aov: 109, rating: 4.4, topDish: "Idly (2)", lang: "Tamil 58%", busy: "1–2 PM" },
  { name: "Town Hall", slug: "town-hall", orders: 3650, aov: 121, rating: 4.5, topDish: "Masal Roast", lang: "Tamil 60%", busy: "8–9 AM" },
  { name: "Saibaba Colony", slug: "saibaba-colony", orders: 3120, aov: 132, rating: 4.7, topDish: "Rajabhojanam", lang: "Tamil 55%", busy: "1–2 PM" },
  { name: "Peelamedu", slug: "peelamedu", orders: 3380, aov: 116, rating: 4.3, topDish: "Veg Briyani", lang: "Hindi 28%", busy: "8–9 PM" },
  { name: "Singanallur", slug: "singanallur", orders: 2890, aov: 119, rating: 4.5, topDish: "Pongal", lang: "Malayalam 22%", busy: "7–8 AM" },
];
...
const SEED_INFO: BInfo = {
  timings: "6:30 AM – 11:00 PM, all days", phone: "0422-xxxxxx",
  ...
  return (
    <div className="wrap">
      <style>{`
        ...
        .head .sub{font-size:12.5px;color:#d9c7a6;margin-top:3px}
        ...
        .rg{border:1px solid #d9c9a6;background:#fffdf7;color:#7a4a24;font-size:13px;font-weight:600;padding:7px 15px;border-radius:20px;cursor:pointer}
        ...
        .who label{font-size:12.5px;color:#9c8e76;font-weight:600}
        select,input,textarea{font-family:inherit;font-size:13.5px;color:#33281a;background:#fffdf7;border:1px solid #d9c9a6;border-radius:10px;padding:8px 11px;outline:none}
        select:focus,input:focus,textarea:focus{border-color:#c79233}
        h2{font-size:15px;font-weight:700;color:#5c3a1c;margin:24px 2px 10px}
        ...
        td{padding:8px 6px;border-bottom:1px solid #f1e9d6}
        ...
        .row .val{width:40px;text-align:right;font-size:12.5px;color:#7a4a24;font-weight:700;flex:0 0 auto}
        .bar{flex:1;height:9px;border-radius:5px;background:#ece2cc;overflow:hidden}
        .bar>span{display:block;height:100%;background:linear-gradient(90deg,#c79233,#9a6a1f)}
        .drop{border:1.5px dashed #d2bd92;border-radius:13px;padding:18px;text-align:center;background:#fffaf0}
        .drop input{display:none}
        .uplabel{display:inline-block;background:linear-gradient(135deg,#5c3a1c,#3e2713);color:#f4ecdb;font-weight:700;font-size:13.5px;padding:10px 18px;border-radius:11px;cursor:pointer}
        ...
        .fld label{font-size:11.5px;color:#9c8e76;font-weight:600}
        .savebtn{background:linear-gradient(135deg,#5c3a1c,#3e2713);color:#f4ecdb;border:0;font-weight:700;font-size:13.5px;padding:10px 20px;border-radius:11px;cursor:pointer;margin-top:12px}
        .menubtn{display:inline-block;background:#fffdf7;border:1px solid #d9c9a6;color:#7a4a24;font-weight:700;font-size:13.5px;padding:10px 18px;border-radius:11px;text-decoration:none}
        .ok{color:#1d7a55;font-size:12.5px;font-weight:700;margin-left:10px}
      `}</style>

      <div className="head">
        <h1>Annapoorna · Supervisor</h1>
      ...
      <div className="container">
        ...
        <div className="grid2" style={{ marginTop: 16 }}>
          <div className="card">
            ...
            <table><tbody>
              {topRows.map((d, i) => (
                <tr key={d.name}><td style={{ width: 24, color: "#9c8e76" }}>{i + 1}</td><td className="b-name">{d.name}</td><td style={{ textAlign: "right" }}>{d.orders.toLocaleString("en-IN")}</td></tr>
              ))}
            </tbody></table>
          </div>
          <div className="card">
              ...
              <table>
                <tbody>
                  {csv.preview.map((r, ri) => (
                    <tr key={ri}>{r.map((c, ci) => <td key={ci} style={ri === 0 ? { fontWeight: 700, color: "#5c3a1c" } : {}}>{c}</td>)}</tr>
                  ))}
                </tbody>
              </table>
        ...
        <h2>Menu</h2>
        <div className="card">
          <a className="menubtn" href={"/annapoorna/admin/" + b.slug}>Manage this branch's menu →</a>
