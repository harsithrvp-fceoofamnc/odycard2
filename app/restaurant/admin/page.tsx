...
// odysra.com/restaurant/admin — HQ admin dashboard (lean MVP).
...
const BRANCHES: Branch[] = [
  { name: "Main Branch", slug: "main-branch", orders: 4120, aov: 124, rating: 4.6, topDish: "Ghee Roast", busy: "8–9 AM", lang: "Tamil 62%" },
  { name: "City Centre", slug: "city-centre", orders: 4480, aov: 109, rating: 4.4, topDish: "Idly (2)", busy: "1–2 PM", lang: "Tamil 58%" },
  { name: "Marina", slug: "marina", orders: 3650, aov: 121, rating: 4.5, topDish: "Masal Roast", busy: "8–9 AM", lang: "Tamil 60%" },
  { name: "Highway Road", slug: "highway-road", orders: 3120, aov: 132, rating: 4.7, topDish: "Rajabhojanam", busy: "1–2 PM", lang: "Tamil 55%" },
  { name: "Tech Park", slug: "tech-park", orders: 3380, aov: 116, rating: 4.3, topDish: "Veg Briyani", busy: "8–9 PM", lang: "Hindi 28%" },
  { name: "Lakeview", slug: "lakeview", orders: 2890, aov: 119, rating: 4.5, topDish: "Pongal", busy: "7–8 AM", lang: "Malayalam 22%" },
];
...
const ALERTS = [
  { kind: "warn", text: "Tech Park lunch (1–2 PM) is running ~18% below the chain average — push the Executive Lunch combo there." },
  { kind: "idea", text: "Lakeview: Malayalam guests are up to 22% — feature Aappam & Idiyappam at the top of the menu." },
...
const SEED_SUPS: Sup[] = [
  { name: "Suresh Kumar", outlet: "Main Branch", phone: "98430 11122" },
  { name: "Lakshmi Ramesh", outlet: "City Centre", phone: "94890 33344" },
];
...
export default function Admin() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("rest_supervisors");
      setSups(raw ? JSON.parse(raw) : SEED_SUPS);
    } catch { setSups(SEED_SUPS); }
  }, []);
  const persist = (list: Sup[]) => {
    setSups(list);
    try { localStorage.setItem("rest_supervisors", JSON.stringify(list)); } catch {}
  };
  const addSup = () => {
  ...
  return (
    <div className="wrap">
      <style>{`
        ...
        .head{background:linear-gradient(135deg,#2b313a,#161a20);color:#f4ecdb;padding:20px 22px}
        .head h1{margin:0;font-size:22px;font-weight:700;letter-spacing:.2px}
        .head .sub{font-size:13px;color:#aeb4bf;margin-top:3px}
        ...
        .rg{border:1px solid #d7dbe2;background:#ffffff;color:#2b313a;font-size:13.5px;font-weight:600;padding:7px 16px;border-radius:20px;cursor:pointer}
        ...
        .ofilter label{font-size:12.5px;color:#8a909c;font-weight:600}
        select,input{font-family:inherit;font-size:13.5px;color:#1f2430;background:#ffffff;border:1px solid #d7dbe2;border-radius:10px;padding:8px 11px;outline:none}
        select:focus,input:focus{border-color:#c79233}
        h2{font-size:15px;font-weight:700;color:#2b313a;margin:26px 0 12px;letter-spacing:.2px}
        .kpis{display:grid;grid-template-columns:repeat(6,1fr);gap:10px}
        @media(max-width:760px){.kpis{grid-template-columns:repeat(2,1fr)}}
        .kpi{background:#ffffff;border:1px solid #e7e9ee;border-radius:14px;padding:13px 14px;box-shadow:0 2px 8px rgba(122,74,36,.07)}
        .kpi .v{font-size:23px;font-weight:800;color:#161a20}
        .kpi .l{font-size:12px;color:#8a909c;margin-top:2px}
        .card{background:#ffffff;border:1px solid #e7e9ee;border-radius:16px;padding:16px 18px;box-shadow:0 2px 10px rgba(122,74,36,.07);overflow-x:auto;-webkit-overflow-scrolling:touch}
        ...
        th{text-align:left;color:#8a909c;font-weight:600;font-size:12px;padding:8px 8px;border-bottom:1px solid #eef0f3}
        td{padding:9px 8px;border-bottom:1px solid #eef0f3}
        tr:last-child td{border-bottom:0}
        .b-name{font-weight:700;color:#161a20}
        .bar{height:9px;border-radius:5px;background:#eef0f3;overflow:hidden}
        ...
        .row .val{width:42px;text-align:right;font-size:12.5px;color:#2b313a;font-weight:700;flex:0 0 auto}
        .row .bar{flex:1}
        .alert{display:flex;gap:10px;padding:11px 13px;border-radius:12px;margin-bottom:9px;font-size:13.5px;line-height:1.4}
        .alert.warn{background:#fff7e6;border:1px solid #f0d9a8}
        .alert.idea{background:#eef4ea;border:1px solid #cfe0bd}
        .alert .ic{font-size:16px;flex:0 0 auto}
        .pill{font-size:11px;font-weight:700;padding:2px 8px;border-radius:10px;background:#eef0f3;color:#2b313a}
        .muted{color:#8a909c;font-size:12.5px}
        a.branchlink{color:#2b313a;text-decoration:none;font-weight:700}
        ...
        .supform .fld label{font-size:11.5px;color:#8a909c;font-weight:600}
        .supform input,.supform select{min-width:150px}
        .addbtn{background:linear-gradient(135deg,#2b313a,#161a20);color:#f4ecdb;border:0;font-weight:700;font-size:13.5px;padding:10px 18px;border-radius:11px;cursor:pointer}
        .rm{background:none;border:1px solid #e2cfcf;color:#b3402e;font-size:12px;font-weight:600;padding:5px 11px;border-radius:9px;cursor:pointer}
      `}</style>
      ...
      <div className="container">
        ...
        <div className="card">
          <table>
            ...
            <tbody>
              {shown.map((b) => (
                <tr key={b.slug}>
                  <td className="b-name"><a className="branchlink" href={"/restaurant/" + b.slug}>{b.name}</a></td>
                  ...
                  <td>{b.busy}</td>
                  <td>{b.lang}</td>
                  <td><a className="branchlink" href={"/restaurant/admin/" + b.slug}>Manage →</a></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="card">
          ...
          <div className="muted" style={{ marginTop: 10 }}>
            ...
            <a className="branchlink" href="/restaurant/supervisor">Open the supervisor view →</a>
          </div>
        </div>
        ...
        <div className="grid2" style={{ marginTop: 16 }}>
          <div className="card">
            ...
            <table><tbody>
              {TOP_DISHES.map((d, i) => (
                <tr key={d.name}>
                  <td style={{ width: 26, color: "#8a909c" }}>{i + 1}</td>
              ...
              {DROPOFFS.map((d) => {
                ...
                return (
                  <tr key={d.name}>
                    ...
                    <td style={{ color: drop > 45 ? "#b3402e" : "#2b313a", fontWeight: 700 }}>{drop}%</td>
                  </tr>
                );
              })}
            </tbody>
