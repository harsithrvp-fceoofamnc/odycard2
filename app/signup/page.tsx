...
export default function SignupPage() {
  ...
  async function withEmail(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(data.redirect || "/signup/details");
    } catch {
  ...
  return (
    <div style={wrap}>
      <div style={card}>
        ...
        <h1 style={{ margin: "6px 0 2px", fontSize: 24, color: "#23262b" }}>Create your account</h1>
        <p style={{ margin: "0 0 18px", color: "#6b7280", fontSize: 14 }}>Use Google or an email — then we&apos;ll grab your hotel details.</p>

        <button onClick={withGoogle} disabled={busy} style={googleBtn}>
          <GoogleG />
        ...
        <form onSubmit={withEmail}>
          ...
          <input style={inp} type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@restaurant.com" autoCapitalize="none" />
...
const btn: React.CSSProperties = { width: "100%", marginTop: 18, padding: "13px", border: 0, borderRadius: 12, background: "linear-gradient(135deg,#5c3a1c,#3e2713)", color: "#f4ecdb", fontSize: 15.5, fontWeight: 700, cursor: "pointer" };
